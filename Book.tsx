...
export const Book: React.FC<BookProps> = (props) => {
    ...
    const zoomStyle = {
        transform: `${props.currentSheetIndex > 0 ? 'translateX(calc(var(--page-width)/2))' : ''} scale(${props.zoomLevel})`,
        transition: 'transform 0.5s ease-in-out'
    };

    const setupStyle = (props.isSetupVisible) ? { transform: 'translateZ(-600px) translateY(-100px) rotateX(20deg) scale(0.9)', filter: 'blur(6px) brightness(0.7)', pointerEvents: 'none' } : zoomStyle;

    return (
        <div className={`book ${props.currentSheetIndex > 0 ? 'opened' : ''}`}
           style={setupStyle as any}>
