...
import { Maximize2, Minimize2, ZoomIn, ZoomOut, Download, RefreshCw, Key, Film } from 'lucide-react';
...
// --- Constants ---
const MODEL_TEXT_NAME = "gemini-3.1-pro-preview";
...
const App: React.FC = () => {
  ...
  const handleAPIError = async (e: any) => {
    ...
    if (msg.includes('429') || msg.toLowerCase().includes('quota') || msg.toLowerCase().includes('resource_exhausted')) {
       alert("Quota exceeded. Please select an API key to continue with unlimited generation.");
       if (window.aistudio?.openSelectKey) {
           await window.aistudio.openSelectKey();
       }
    } else if (msg.includes('Requested entity was not found') || msg.includes('API_KEY_INVALID')) {
       if (window.aistudio?.openSelectKey) {
           await window.aistudio.openSelectKey();
       }
    }
  ...
  // Helper to retry API calls on 429
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        if (msg.includes('429') && i < retries - 1) {
          console.log(`429 Hit. Retrying in ${delay}ms... (Attempt ${i + 1}/${retries})`);
...
`;
    try {
        const ai = getAI();
        const res = await withRetry(() => ai.models.generateContent({ model: MODEL_TEXT_NAME, contents: prompt, config: { responseMimeType: 'application/json' } }));
  ...
  const generateImage = async (beat: Beat, type: ComicFace['type']): Promise<string> => {
    ...
    try {
        ...
        handleAPIError(e);
        // Return a generic colored placeholder to avoid getting stuck in "PRINTING..." loop
        return `https://placehold.co/500x750/111827/FFFFFF?text=Generating...`; 
    }
  };
  ...
  const generateBatch = async (startPage: number, count: number) => {
      ...
      try {
          // Parallelize generation for speed; withRetry will handle rate limits and stagger automatically
  ...
  const downloadPDF = () => {
    ...
    pagesToPrint.forEach((face, index) => {
        ...
        // Overlay Text components if they exist
        if (face.narrative) {
            const { caption, dialogue } = face.narrative;
            
            // Draw Caption Box (Top)
            if (caption) {
                doc.setDrawColor(0);
                doc.setFillColor(255, 255, 240); // Paper yellow
                doc.rect(20, 20, PAGE_WIDTH - 40, 60, 'FD');
                doc.setLineWidth(2);
                doc.rect(20, 20, PAGE_WIDTH - 40, 60, 'D');
                
                doc.setFont('courier', 'bold');
                doc.setFontSize(10);
                doc.setTextColor(0);
                const splitCaption = doc.splitTextToSize(caption, PAGE_WIDTH - 60);
                doc.text(splitCaption, 30, 40);
            }

            // Draw Dialogue Box (Bottom)
            if (dialogue) {
                const diaY = PAGE_HEIGHT - 100;
                doc.setDrawColor(0);
                doc.setFillColor(255, 255, 255); // White bubble
                doc.roundedRect(40, diaY, PAGE_WIDTH - 80, 70, 10, 10, 'FD');
                doc.setLineWidth(1.5);
                doc.roundedRect(40, diaY, PAGE_WIDTH - 80, 70, 10, 10, 'D');

                doc.setFont('helvetica', 'normal');
                doc.setFontSize(11);
                doc.setTextColor(0);
                const splitDialogue = doc.splitTextToSize(`"${dialogue}"`, PAGE_WIDTH - 100);
                doc.text(splitDialogue, 50, diaY + 25);
            }
        }
    });
  ...
  return (
    <div className="comic-scene">
      ...
      {isStarted && !showSetup && (
        <div className="fixed top-4 right-4 z-50 flex flex-col gap-2 p-2 bg-black/60 backdrop-blur-xl rounded-2xl border border-white/20 shadow-2xl transition-all hover:scale-105">
           <button onClick={() => handleZoom(0.1)} className="p-3 bg-white/10 hover:bg-white/30 rounded-xl text-white transition-all flex items-center justify-center group/btn" title="Zoom In">
             <ZoomIn size={22} className="group-hover/btn:scale-110" />
           </button>
           <button onClick={() => handleZoom(-0.1)} className="p-3 bg-white/10 hover:bg-white/30 rounded-xl text-white transition-all flex items-center justify-center group/btn" title="Zoom Out">
             <ZoomOut size={22} className="group-hover/btn:scale-110" />
           </button>
           <button onClick={toggleFullscreen} className="p-3 bg-white/10 hover:bg-white/30 rounded-xl text-white transition-all flex items-center justify-center group/btn" title="Toggle Fullscreen">
             {isFullscreen ? <Minimize2 size={22} className="group-hover/btn:scale-110" /> : <Maximize2 size={22} className="group-hover/btn:scale-110" />}
           </button>
           <button onClick={() => setIsCinematic(true)} className="p-3 bg-white/10 hover:bg-yellow-500/40 rounded-xl text-white transition-all flex items-center justify-center group/btn" title="Motion Comic (Play)">
             <Film size={22} className="group-hover/btn:scale-110" />
           </button>
           <div className="h-px bg-white/10 my-1 mx-2" />
           <button onClick={downloadPDF} className="p-3 bg-white/10 hover:bg-blue-500/40 rounded-xl text-white transition-all flex items-center justify-center group/btn" title="Download PDF">
             <Download size={22} className="group-hover/btn:scale-110" />
           </button>
           {typeof window !== 'undefined' && window.aistudio?.openSelectKey && (
               <button onClick={() => window.aistudio?.openSelectKey()} className="p-3 bg-white/10 hover:bg-yellow-500/40 rounded-xl text-yellow-400 transition-all flex items-center justify-center group/btn" title="Set Custom API Key">
                 <Key size={22} className="group-hover/btn:scale-110" />
               </button>
           )}
           <button onClick={resetApp} className="p-3 bg-red-500/20 hover:bg-red-500/40 rounded-xl text-white transition-all flex items-center justify-center group/btn" title="Reset Story">
             <RefreshCw size={22} className="group-hover/btn:scale-110" />
           </button>
        </div>
      )}
